...
/**
 * Simple responder flow delegate that verifies the transaction
 * and ensures all output states are of a certain type(s) before signing.
 */
class SimpleTypeCheckingResponderTxStrategy(
        vararg val contractStateTypes: Class<*>
) : PartitureResponderFlowDelegateBase(), ResponderTxStrategy {
    ...
    override fun createSignTransactionFlow(): SignTransactionFlow {
        ...
        return object : SignTransactionFlow(
            ...
            override fun checkTransaction(stx: SignedTransaction) = requireThat {
                ...
                // Ensure the transaction only has outputs of the desired type(s)
                val msg = contractStateTypes.joinToString(
                        separator = ", ",
                        prefix = "Type of transaction output state(s) must be one of [",
                        postfix = "]"
                ) { it.name }
